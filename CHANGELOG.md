...
- **The wiki** (`canonical_genealogy/wiki/`, 9 pages): viewer-app, data-layer, substrate, review-tool, toys-specs (agent-onboarding: contracts, gotchas, modify-safely checklists, verification steps) + **UI_GUIDELINES.md** (the design language as law — honesty conventions are correctness rules: proxies labelled, data-absent draws nothing) + **REVIEW_THROUGH_THE_FRAMEWORK.md** (pin = forcing event; ask/give = A−/A+ fold; status = weld lifecycle; delete = retire-not-destroy — §7 honestly splits real structure from convenient vocabulary) + **PIPELINE_GAP_ANALYSIS.md** (current shape vs scale shape; staged path; keep-list: single-file viewers stay) + **PIPELINE.md** (the 7-station conveyor: pin → wiki-armed triage → scouts → guideline-bound patch → 6 verification gates → give on the pin → Pav verifies in-frame; model tiers per role).
- **Review tool v3:** full-slice capture — the composite PNG now rasters DOM panels/bars via z-index-sorted foreignObject (the bottom bar Pav commented on is IN the picture; review chrome categorically excluded; `__review.lastCapture` makes gaps inspectable); **exact-slice replay** via `__getReviewState`/`__applyReviewState` hooks in viewer_v3 (specimen, year, dials, toggles, every panel's position/collapsed/floating + symmetric float↔re-dock); **pins are tickets**: edit / follow-up notes / retire (records never destroyed), ask/give/status lifecycle (open→acknowledged→answered→applied→verified, color-coded pins, history log; PATCH+DELETE endpoints).
- **Pin #1 processed by the pipeline itself:** the ask ("make this menu collapsable and dragable like the other overlay panels") implemented — the transport bar is a managed panel (docked by default, grip+chevron, floats on first drag, replay-restorable) — give recorded, status **applied** with a live headless verification note. Agents cap at *applied*: **verified is Pav's click**, in-frame, comparing old capture vs the updated build.
- Opus: all five asks MET from code; framework-POV doc "does NOT overclaim"; gap analysis "honest about what does not exist." All flagged defects fixed in finalize and re-verified live (re-dock symmetry, capture z-order, toolbar exclusion, accent-color raster fidelity).

---

## 2026-06-10 — THE FRAMEWORK RENDERS ITSELF: reflexive self-specimen + CLAIM_LIFECYCLE + viewer v3 (Pav's steer; Opus: "no inflation found — aggressively self-deflationary")

Pav: "put the agnostic framework into the viewer, at least the current slice… sure to bring insight" + the claim-lifecycle reframe (a claim is never killed — DEMOTED / DORMANT / FRICTION-marked as a dated record; the conjecture is a PARENT function, the experiment its CHILD — children die, parents persist, and the dead-children tally is the honest falsification-pressure gauge). 10-agent workflow (4 repo-miners + lifecycle design + specimen build + self-narration + viewer wire / Opus skeptic / Fable finalize). New: `specimens/agnostic_framework.json` (8th specimen, Tier-3 reflexive, surfaced for ratification — NOT ratified canon), overlay + self-narration + 48 provenance facts (repo-as-primary-source; 48/48 narration refs resolve), `candidates/CLAIM_LIFECYCLE.md`, `viewer_v3.html` (lifecycle render states: demoted ▾ dim, dormant dashed, friction + dead-children ✕N badge; the reflexive banner). List stays 9 — the specimen explicitly refuses to count itself as a 10th.
- **What the self-render revealed:** (1) **convergence-by-independence cuts both ways** — the framework's prized independent-convergence evidence is also the strongest argument against its central object's novelty (the pushout is Goguen's known colimit, reached by a second route); S_structure.agreed=false. (2) **Instrument ~0.85 / field ~0.4** — the split verdict, rendered as a first-class walked-back node. (3) **The thinnest place is the synergy gate, measured:** parents-produce-W_C carries **4 dead children, 1 live-but-never-run (gain_v2), 0 real-corpus successes — pressure HEAVY**. (4) The 1/f parent shows the healthier contrast: 3 dead children → parent DEMOTED rather than endlessly re-childed. (5) The tally is a falsification-pressure **gauge, not yet a mechanism** — the auto-park rule (Meditation #11) is proposed-not-adopted; that gap is the live open friction, published not papered. (6) Honest gaps forced into the open: Gärdenfors unwelded; novel residue only 0.12 of theory-DNA; "weld" appears in ZERO continuations (render-vocab vs source-vocab gap, disclosed).
- **CLAIM_LIFECYCLE.md** formalizes the reframe (states mapped to existing schema enums; the gain_v2 case worked end-to-end; anti-gaming: demotions append-only + dated, revivals don't reset counts, "refine the estimator again" costs +1 dead child) + §6 folds the Opus special-pleading verdict: "the reframe does NOT rescue the claim; it makes its weak standing auditable."
- Opus: 6/6 provenance spot-checks pass (several verbatim), no inflation, accept Tier-3; external GPT+Gemini pass on the self-specimen owed before anything hardens. Verified live: lint 0 violations ×8 specimens, reflexive banner correct, no regression on the 7 base specimens.

---

## 2026-06-10 — VIEWER v2 (crash fix, tree dial, weather, bedrock L0, narration-off-the-substrate) + the MEDITATION (what questions should we be asking)

Pav's v1 first-pass round, 16-agent workflow (Fable work / Opus skeptic / Fable finalize). New: `viewer_v2.html`, `narration/*.narration.json` (7), `candidates/MEDITATION_QUESTIONS.md`, `canonical_genealogy/PROXY_SPEC.md`; substrate `_summary.json` mislabel fixed (22 = dispute *records*; disputed best-values = 11). Tier-3; list stays 9.
- **Crash diagnosed + fixed:** rank-1 was per-frame innerHTML rebuilds of two panels at 60fps with a never-pausing rAF loop (not the gradients — rank 2, also fixed). v2: document.hidden pause, sprite-cached node renders (no per-node gradients), panels rebuild only on change (~5Hz throttle + signature), memoized mass, visible-set-once physics, frame-time quality ladder, DPR cap. Opus: "the crash fix is the strongest part of this build."
- **K1 conceived smear** (deliberate coreless pre-formulation haze); **R2 orientation dial** (vertical growth, time top→bottom) + **weather visible** (pull/squeeze arrows from real forcing_events only — 3 specimens carry none and draw none) + minimal causality-loop ring (full nested-loop view deferred, disclosed); **R3 bedrock/L0** accumulating stratum (canon theories + instruments graduate in; anchor lines disclosed as nearest-by-date proxy, NOT attested dependency); **R4 draggable/collapsible panels**; **R5 click detail panel** with full text + per-field substrate provenance (certainty, verification bucket, source, fact_id); **R6 narration track** — 336 substrate refs across 7 specimens, 0 phantom, 6/6 spot-checked segments faithful (the AlexNet 15.3%/28.2% beats literally carried by facts); the one unsourced numeric (Weber-Kohlrausch value) removed.
- **Self-honesty test added:** an embedded render-faithfulness linter (#lint) asserts every narration ref resolves, sizes finite, solidity in [0,1], provenance rows sourced — PASS ×7. Proxy weights extracted from magic numbers into versioned PROXY_SPEC.md.
- **THE MEDITATION** (5 lenses + Opus's "nobody asked" fold): ranked 12 questions in `candidates/MEDITATION_QUESTIONS.md`. The bets: (1) the real-merge gain_v2 pilot with a PRE-COMMITTED KILL THRESHOLD (three refutations have been absorbed as estimator-not-claim failures — "a vocabulary that cannot lose" until the claim itself can fail); (2) the provenance-DISJOINT verification test (substrate is 77% Wikipedia — is 32.7% verified triangulation or an echo?); (3) should the viewer SHOW the failed welds (7 winners = a hall of fame by construction); plus the presenter fork (frozen-auditable vs live-generated, fabrication-rate eval needed) and the instrument question (name one render that could be wrong in a way that teaches).
...
- **Physics verdict: fragments-on-solid-rails.** No unified physics-of-the-latent exists (4 model families agree). Pav's recursive loop = **data assimilation** (Kalman→particle filters→EnKF/weather→4D-Var); the sharp/fuzzy ratio = posterior covariance/ensemble spread. Pav's divergent-rules+spectrum instinct = **EFT + renormalization group** (+ Anderson emergence), with the load-bearing **RG-semigroup caveat**: coarse-graining destroys information, slices→deep-law is NOT guaranteed; and RG is single-axis — no off-the-shelf RG over a knowledge/meaning axis.
- **Genuinely-new residue (honest):** the integration layer; observer-relative SOLIDITY (no analog in the dynamics rails; epistemology has adjacent analogs — perspectival realism, active inference, QBism — so the new move is making it an *instrument parameter*); frame/instrument-as-estimand; multi-axis frames. All gated on gain_v2 on a real corpus, pre-registered. Shared external risk: no Hamiltonian/metric in idea-space → metaphor until operationalized.
- **Covered-ground audit (Pav was right "some of this is covered"):** squeeze/pull = cont 21 + D6; attract/repel + flavours = cont 13 charge canon (cont 15 superseded the Cayley table to ASYMMETRIC — Opus catch); touch-verbs (tap/stroke/poke/stomp) already an atlas vocab. New bits: the unified graded force-SPECTRUM, size→force-coupling magnification, the dissolution-MIRAGE rule, span-as-solidity criterion (a new criterion on the existing cont-22 four-layer stack — Opus corrected "never bundled").
- **VIEWER v0 WORKS** — verified in a real browser: all 7 specimens render; timeline scrub + play; latent band (fuzzy idea-blobs, radius=size, blur=fuzz, opacity=solidity-proxy) anchored by stems to the physical band (institutions/people); welds fire as the timeline crosses them; QM+GR renders the **OPEN — unconsummated weld** dashed splat (Opus caught the false-2026-fusion bug; fixed; verified); sharp/fuzzy meter is specimen-meaningful (Deep Learning 94% vs Quantum Gravity 47%); 4 spectator presets; mass→force + mirage sliders; honest proxy-disclosure in the legend. Opus-flagged bugs all fixed + Node-harness verified (YYYYs decade dates; forcing-target resolution 100% on all 4 forcing specimens).
- Deferred honestly: D6 friction render (zero specimen data), charge-driven forces (no a_charge anywhere), full harness-indexed solidity, the forward-simulation loop (display-only ratio for now).
...
## 2026-06-11 — Brief 2 methodological upgrades folded (no scope expansion)

Three line-level upgrades to HANDOFF.md Brief 2 (Stage 2 outcome-blind base-rate census) honoring the three Tier-3 surfaces from the 2026-06-09/10 round that strengthen the brief's discipline without broadening its substantive subject:

- **(a) Honest scope-naming.** "Why this is the right next empirical move" now states Brief 2 satisfies **one of two outcome-blind base-rate censuses** the framework owes. The sibling = the **convergence-9 list census** (the 9-item list was assembled by hunting named instances; same survivorship + named-search confound as §5.1-R's adversarial-niche corpus). Flagged in `specimens/agnostic_framework.json` `discrepancies[]` + `MEDITATION_QUESTIONS.md` #7. Brief 2 isn't over-positioned as "the" census the framework owes; it satisfies one of two.

- **(b) Pre-registration discipline upgrade.** Step 1 now requires a SHA-hashed YAML/JSON lock file (`pilots/adversarial_bar_a/lock_brief_2.yaml`) alongside the markdown pre-registration, with **called shots per heredity stratum** + per-substrate predicted PASS/FAIL. The lock file goes on its own git commit (no other changes); SHA256 recorded in markdown header + commit message; at result-commit the lock hash is recomputed and `git merge-base --is-ancestor <lock_commit> HEAD` verified true. This is the frame-lock protocol §7 discipline demonstrated working on controlled ground truth 2026-06-09 (`frame_lock_pilot_RESULTS.md`, sha256 `747ed7f2…3713d` recomputed-matching), strengthening Pilot 2's content-only pre-reg with commit-graph tamper-evidence.

- **(c) Post-FAIL vocabulary.** The if-FAILS guardrail now references `candidates/CLAIM_LIFECYCLE.md` parent-stays / child-dies / +1-tally discipline. If census fails: §5.1-R **parent goes to PARKED** with friction-logged dated record + revisit trigger; this census's **specific operational instantiation is the dead child** (chosen detection proxies, chosen Chao1 implementation, specific "first opportunity" threshold) — retired with dated reason; bounded claim's text stays frozen (the §2.1 anti-gaming rule: "refine the estimator again" costs +1 dead child rather than resetting). The §5.1-R lineage already carries one prior child-retirement (the §5.1 strong full-spectrum reading narrowed to §5.1-R as the surviving form); if this census fails, that's the second narrowing of the same parent's lineage.

Files-to-read list for Brief 2 extended to include `frame_lock_protocol_DRAFT.md` §7, `frame_lock_pilot_RESULTS.md` (worked example of the lock mechanic), `frame_lock_data/lock_pilot.yaml` (what a real lock file looks like), `candidates/CLAIM_LIFECYCLE.md`, and `specimens/agnostic_framework.json` discrepancies[].

The brief's substantive subject is **unchanged** — still §5.1-R bounded-claim Bar A satisfaction via outcome-blind Chao1 + heredity-stratified prediction. Other 2026-06-09/10 Tier-3 candidates (observer_frame_kernels, parents_produce_WC_FORMALIZATION, frame_lock_protocol body, canonical_genealogy SCHEMA + 8 specimens, latent_cosmology, latent_physics, synergy_vs_pid gain_v2, agnostic_units, Latent Olympics build) **NOT folded** — they're orthogonal to the §5.1-R substrate question and remain on their own tracks per Pav steer.

Nothing compiled to canon; convergence list stays 9; candidate doc still byte-identical; no tier advanced; no continuation written.
...
- `candidates/observer_frame_kernels_DRAFT.md` — the frame insight (wrappers in observer-global-kernel frames {time,space,knowledge,meaning}); **dissolves the cont 25 "rocks touching" falsifier** (two rocks + the time-kernel → strata = a W_C); makes parents-produce-W_C **frame-relative**; proposed canon placements + the dual-kernel-resolved-2+2 hypothesis (flagged verify-not-claim).
- `candidates/parents_produce_WC_FORMALIZATION_DRAFT.md` — **PPWc(W_A,W_B | frame)** = a non-degenerate **pushout** (universal-property falsifier) **gated by synergy** (K-non-reducibility generalizing BES Theorem 4.4; MDL/bits surrogate). 2-agent dynamic workflow + a validation set (passes the 9 with explicit pushout squares for LCAO/ACMP/BES; **fails cymatics in every frame**; passes two-rocks-in-time, ∅ for rocks-in-space; fails additive-blend on synergy=0).
- **Cross-model review (GPT-5.5 + Gemini, first live use of the local Codex/Gemini CLIs): TRIPLE CONVERGENCE** — both confirm the pushout+synergy fix closes the gap they flagged, both rate it sound-with-caveat, both independently name the same remaining hole (the frame/coarse-graining must be **locked before evaluation** = the internal flag + audit v08 §C7 lock-before-data). Bar to promote: compute an MDL synergy estimate (model-merging weight-space) + pin per-frame morphisms + lock the frame-resolution. No timeline/JSON bump (working drafts).
...
**§7 added (same session) — Pav's 2026-06-08 reframe** (governance grounded in established science as a faction-proof *measuring ruler*; public Olympics as *squeeze* + *classifier / differential-correlation / reservoir-computing substrate*), developed + stress-tested via a second 3-agent workflow. Outcome: the strongest move in the design so far, but both blockers **NARROW, neither closes** — governance gets a faction-proof anchor (cont 22 §3) yet the reflexive gap relocates to the positioning-function + review-panel (and the ruler is silent for the soft-band frontier conjectures — governance = two rulers); the public crowd gets a genuinely non-vestigial job (it is the reservoir's **exogenous input**, the cure for the self-validation trap) but only **conditional on** a pre-registered baseline-beating readout, which is itself coupled to governance (RC needs a fixed reservoir; the population is driven by the contested rating function). Net bottom line moves from "vestigial/gated" to "load-bearing conditional on one unrun experiment, on an unclosed governance residual." Sketch now §1-8.
...
- **Cont 28 — Agent-substrate as next observer-class · The supersede dynamic at the discovery substrate · Framework as flower.** Captures Pav's "flower for AI agents" strategic insight as canon at multiple tiers.
- **Tier 1 epistemological canon promoted** in §2: the discovery substrate of the internet is undergoing a structural supersede dynamic. Old substrate (gatekeeper algorithms) persists as procedural-root stubs (cont 25 §1 supersede branch); new substrate (personal AI agents) increasingly uses the old as one tool among many. Evidence: Reading 01 documented CTR-to-source-sites drop (34-46%) when AI Overviews appear; ChatGPT/Claude/Perplexity reached substantial user adoption 2024-2025; the marginal research-adjacent query is increasingly agent-mediated.
- **Tier 2 candidate proposed** in §5: personal AI agents are a load-bearing internal organ of the cont 22 internet-as-observer-class, themselves operating as observer-substrate at a sub-ring. Candidate evidence: agents have observable harness + wrapper + inner sandbox + action-space mapping cleanly to framework primitives; observable A⁺/A⁻ compile loops via constitutional AI / RLHF / RAG; observable canon-stack mapping (model weights = kernel canon, system prompts + alignment fine-tunes = compiled canon, in-flight reasoning traces = canon artefact, working agent answering query = function canon). Caveats: single-session vs persistent canon is real architectural difference; analogical stretching may not survive empirical examination.
- **Tier 3 speculations explicitly NOT promoted but named** in §5: agents may form their own L0 with evolved failsafes; agent-substrate may eventually become its own cont 23 ring; persistent agent identity across sessions may dissolve the single-session caveat. Held for later review.
- **Five scoreable predictions** with 12mo / 18mo / 24mo / 36mo / 60mo horizons covering agent-mediated query share, outreach discovery paths, citation behavior, agent-as-observer-class construct study appearance, and acceptance/rejection of the supersede claim itself.
- **Six places this changes the framework** (§6) + five places this does NOT change (§7) per cont 27 §2 discipline.
- **Connects to four prior continuations:** cont 16 (carrier-before-canon), cont 18 (procedural lineage), cont 22 (internet-as-observer), cont 25 §1 (supersede branch), cont 26 §3 (L0 evolved failsafes), cont 27 §2 (three-tier framework). Every connection named with the specific mechanism.

### Added (JSON endpoints — subagent A + B generated)
- **/manifest.json** — top-level catalog with all framework endpoints, agent-discovery URLs, feeds, key pages, interactive artifacts, tier-discipline note
- **/primitives.json** — 76 entries (43 canon / 30 candidate / 3 speculative) with status + tier + summary + counter + related URLs. Generated by extracting `const PRIMITIVES` from index.html
- **/readings.json** — 22 entries with dates, summaries, predictions, source URLs, synthesized tier_tags
- **/candidates.json** — 2 entries with promotion status (energy-floor failsafe promoted-to-canon 2026-05-27; aux-channel-l0-occlusion speculative)
- **/speculations.json** — 2 entries with full prediction horizons + promotion bars + next-review date (2026-11-27)
- **/continuations.json** — 25 entries numbered 2-27 (skipped _RAW and _checkpoint files; included 09b with number 9.5 for ordering)

### Added (RSS + Atom feeds — subagent C generated)
- **/timeline/feed.xml** (RSS 2.0) — 20 newest timeline entries with CDATA-wrapped HTML descriptions, full Pav steers, tag categories
- **/timeline/atom.xml** (Atom 1.0) — same content in Atom format with proper `tag:` URI scheme IDs and self/alternate links

### Updated
- **/for-agents/index.html §02** — added 8 new endpoint cards covering all JSON endpoints + RSS/Atom feeds. Page now lists 11 total canonical agent surfaces (/llms.txt, /llms-full.txt, /sitemap.xml, /robots.txt, /CITATION.cff, /CHANGELOG.md, /manifest.json, /primitives.json, /readings.json, /candidates.json, /speculations.json, /continuations.json, /timeline/feed.xml, /timeline/atom.xml)

### Three parallel subagents
- Pav's "spin up some sub agents to accelerate" steer was executed cleanly. Three general-purpose subagents in parallel handled mechanical transformation work (JSON extraction + RSS/Atom generation) while main thread wrote cont 28. Total wall-time ~10 minutes for what would have been 30-45 minutes sequential. Demonstrates the framework's own agent-substrate-as-load-bearing-internal-organ claim (cont 28 §5) operating in its own production.

### Why this round matters
- **Closes out the 2026-05-28 day's arc** — outreach drafts → SEO pass → Search Console → reframe → agent-substrate optimization shipped end-to-end → canonical framing captured in cont 28.
- **The framework is now fully agent-substrate-optimized.** Every machine-readable surface exists (llms.txt, JSON endpoints, RSS feeds, expanded robots.txt). The framework's content is structurally machine-parseable, tier-tagged at intake, provenance-explicit, densely cross-referenced, available via canonical agent-discovery surfaces — the five properties cont 28 §4 names as defining agent-substrate optimization.
- **The framework's strategic claim about the discovery substrate is now canon** (cont 28 §2 Tier 1). The supersede dynamic from gatekeeper algorithms to agent-mediated discovery is observable, dated, and framework-vocabulary-mapped. If the claim doesn't hold under 5-year observation, cont 28 §2 gets demoted per cont 27 §3.
- **Agent-substrate as observer-class candidate** is named at Tier 2 (cont 28 §5) extending cont 22 internet-as-observer. This is the framework's most concrete ring-extension move since cont 22.
- **The framework is producing data about how it operates in the new substrate**, not just describing the new substrate. Self-reflexive, consistent with "be itself, not describe itself" stance.

### Carry-forward (queued for cont 29+ and beyond)
- **Cont 29 candidate:** construct study on agent-substrate as observer-class candidate (2027-era when more empirical material exists)
- **Carrier-attraction as candidate primitive** — the "flower" metaphor sharp enough to deserve naming as framework primitive; the property by which a canon-bearing artifact is structured to attract carriers that propagate it onward
- **Agent-substrate evolved failsafes** as research direction (cont 26 §3 extended to agent ring)
- **Framework's own observability of agent-substrate discovery rate** — currently no measurement infrastructure distinguishing agent-mediated vs direct-search arrivals. Would let P1/P2 predictions score empirically.
- **Open question for cont 30+:** if agent-substrate matures into observer-class in cont 22 sense, does it form its own internal four-canon stack? Initial candidates listed in cont 28 §10.
- **Track P1-P5 predictions** at respective 12/18/24/36/60mo horizons per cont 27 §3 procedure
...
### Added (canon-level)
- **`continuations/25.md`** — ~6,500 words across §0–§11. Promotes the one-system-with-phases model of wrapper-overlap to canon, replacing cont 24 §2's three-discrete-outcomes trichotomy (union/repulsion/interface). Cont 24 §2 entry preserved as provenance — it was the v0 of the thinking that produced cont 25. Trigger: Pav's eleven steers across the diagram refactor: 'lets work a diagram' → 'can we have it programmatically animating to show the phases?' → 'what's that last one? the intermidate?' → 'when two things cones overlap there's always a 3rd new wrapper emergance... so its not three things we want to show but one system in various states of play' → 'the latent space math is not 1+1 = 2 ist more like (1+1) = 3 or 1^+- depending on the phase' + 'its like the hairy sphere the parents retain little stubs and are like roots' → 'the W A W B circles shouldn't move, thye should expand' → 'need W_A W_B circles to overlap over each other to create the nesting space for W_C... they should start smaller and positioned next to each other' → 'when W_A and W_B membranes touch each other thats the first contact, the push against each other and negotiate try to render the full shape of one another... show them as )( with the line width fat where they meet and then thining out... its a circle that's never closed' → 'lets treat the initial canvas frame as a quantised space, based on ratios and percentages... mathematical if it isent already, not fed in data points but a recursive live formula... lets send some scouts and come up with candidates' → 'the bounded frame should be local space where it happens, lyrs move the phases to the bottom... interesting finding lets take note and then try your sugestion and an alt one that a subagent can make and see it side by side' → 'lets update the site and project files to capture where we landed with providence'.
- **One-system-with-phases model promoted to canon (replaces cont 24 §2 trichotomy)** — seven lifecycle phases: pre-overlap → expansion → wake/negotiation → fuzzy/lens → W_C conceived → emerged → active. Four branches from active: passive, dormant, supersession, break-apart, with recurrent loop.
- **Hairy-sphere topology promoted to canon** — W_C is conceived inside the Venn-lens overlap, then expands beyond the lens into the parents. Parts of parents not covered by W_C persist as visible STUBS — procedural roots / receptor-hooks / containers of dormant compiled originals (W_A° and W_B°).
- **)( open-arc wrapper representation promoted to canon** — wrappers are never-closed arcs with variable stroke width: fat at contact (high observability), thinning toward the back, vanishing entirely at 180° from contact. Replaces closed-circle rendering used in pre-v7 diagrams.
- **Membrane negotiation at first contact promoted to canon** — wake/first-contact is active, not passive. Membranes push against each other rhythmically. Each tries to render the other's full shape (which it never can per K̂ ⊂ G from cont 23). Front-arc stroke pulses during wake phase.
- **Non-additive latent math promoted to canon** — phase-dependent arithmetic: (1+1) = 2 pre-overlap, 2+ε wake, ⌒ fuzzy/lens, ⌊3⌋ proto, (1+1) = 3 emergent, (W_A + W_B) ⋈±∓ W_C = 3 with stubs° active, held-3 passive, ghost-3 dormant, 1⁺ supersession, 1⁻ break-apart recurrent. Latent-space arithmetic is non-additive; result depends on phase and bridge viability.
- **Cross-substrate convergence promoted to canon** — wrapper-overlap dynamic has been independently formalized in SIX research traditions in 2024–2026: (1) LCAO/MO theory (quantum chemistry, match 44/50, cleanest static math); (2) cell fusion (cellular biology, match 40/50, Nov 2024 topological-defect localization finding); (3) symbiogenesis/secondary endosymbiosis (organismal biology, match 46/50 — highest of six, nucleomorph as literal stub° empirical confirmation); (4) creole linguistic genesis (linguistics, match 41/50, Mufwene feature pool); (5) conceptual blending (cognitive science, match 42/50, Fauconnier-Turner + Goguen); (6) model merging (machine learning, match 42/50, TIES/DARE/mergekit/Arcee Fusion). Same architectural pattern, six substrate scales, all formalized in 2024–2026. Same shape as cont 15's four-carrier convergence at the wrapper-overlap level. Framework's wrapper-overlap is not idiosyncrasy — it is architecturally validated pattern. Findings synthesized from three parallel research scouts dispatched during the v8 diagram refactor.

### Added (artifact)
- **`artifacts/wrapper_overlap_animated.html`** — standalone v9 of the diagram saved to artifacts. Full-page HTML with framework topbar, hero header, the v9 widget (animated dynamic + lifecycle graph + mapping comparison table), provenance section listing all nine iterations, three explanatory context sections. Hypothes.is wired. Mapping toggle switches all element labels live between Framework / LCAO / Model Merging vocabularies. Compatibility slider modulates bridge viability. Live state readout shows growth, contact, negotiation, emergence, maturity, r in real time.
- **Diagram 07 v1→v9 iteration trail** — nine versions of the diagram produced in one day, each collapsing a wrong assumption: v1 static trichotomy → v2 animated parallel → v3 one-system-with-phases → v4 hairy sphere + non-additive math → v5 fixed-centre expansion → v6 Venn-lens nesting → v7 )( open arcs + membrane negotiation → v8 quantised + formula-driven cascade → v9 layout fixed + mapping toggle. Trail documented in cont 25 §1 and in artifacts/wrapper_overlap_animated.html provenance section.

### Changed
- **`index.html` PRIMITIVES §02** — wrapper-overlap-dynamics-trichotomy entry rewritten as one-system-with-phases (cont 25 supersedes cont 24 §2). New cross-substrate-convergence entry added as canon. Both link to cont 25 and to the animated artifact.
- **`index.html` Map section** — added link to /artifacts/wrapper_overlap_animated.html alongside /atlas/, /notation/, /formalization/ as the framework's load-bearing diagrams.

### Method
- Single-session sequential build with three parallel research scouts (mathematical/physical, organizational/biological, linguistic/ideational candidates for wrapper-overlap mapping). Scouts returned convergent findings: six independent research traditions formalize the same dynamic in 2024–2026. Top picks by domain: LCAO (cleanest math), symbiogenesis (cleanest biological phase taxonomy, with nucleomorph as empirical stub°), model merging (live observable engineering lifecycle, active 2025 research front). Subagent dispatched again in this round to build the Model Merging mapping as alt to LCAO. Result: both mappings shipped as toggle options in v9 widget; convergence finding promoted to canon as cross-substrate validation.

### Carry-forward (still pending)
- Static SVG fallback for /diagrams/07_wrapper_overlap_dynamics.svg — currently only animated HTML exists; static SVG for places that can't render JS.
- Diagram 07 case study showing all nine iterations the way diagram 06 has its case study at index.html#case-study.
- Refactor existing construct studies (marriage, religion, language, nation-state, internet) to use cont 25 one-system-with-phases vocabulary instead of cont 24 §2 trichotomy.
- Operational measurement of W_C emergence threshold (cont 25 §9 OE1) — at what compatibility does the bridge reliably consolidate vs stall at wake?
- Cross-substrate predictive transfer test (cont 25 §9 OE4) — does the cell-fusion finding (Nov 2024, fusion localizes at topological-stress defects) predict that model merging at sign-conflict hot-spots produces stronger consolidation?
- Outreach to researchers in 3+ of the six convergence traditions (audit v04 had outreach queued; now lifted with the convergence finding as the natural pitch).
