#!/usr/bin/env python3
"""
V11b -- the MEANING / KNOWLEDGE rung with a compute-matched novel predicate.
This control isolates concept de-amortization from predicate execution difficulty.

We swap 'prime' for a novel predicate 'WURF' (coprime-to-D / non-remainder rule)
designed to have a comparable trial-division search tree to primality testing,
while selecting the identical unique needle.
"""
...
# Word-count matched definitions (29 words each)
DEF_PRIME = "Call a number {W} iff it is an integer at least 2 whose only positive divisors are 1 and the number itself. This is a property of the number."
DEF_NOVEL = "Call a number {W} iff it is at least 2, not even (unless 2), and for each d in {{3,5,7,11,13,17}}, the remainder of division by d is not 1."
...
def _make(seed):
    """6 chains, all-distinct finals, EXACTLY one prime (needle), which is also the UNIQUE WURF final."""
    rng = random.Random(f"V11b|{seed}")
    for _ in range(50000):
        ...
        if len(primes) != 1:
            continue
        needle = primes[0]
        if not _is_wurf(needle["final"]):
            continue
        non = [c for c in chains if c is not needle]
        if any(_is_wurf(c["final"]) for c in non):
            continue
        return {"chains": chains, "needle": needle}
    raise RuntimeError(f"no V11b structure for seed {seed}")
...
def gen_cell(cond, seed):
    ...
    rng = random.Random(f"V11bord|{seed}")
    ...
    if cond == "NAMED_BARE":
        ...
        body = _verdict_body(order, _is_wurf)
        prompt = (f"{head}\n{DEF_NOVEL.format(W='ZILP')}\nEach line shows a computation, its final result after "
                  f"'=>', and a verdict. Report the value of the ONE whose verdict is PASS. Output ONLY that integer.\n\n{body}\n\nThe integer:")

    compute_free = cond.startswith("F0")
    needle_line = (_line0(needle) if compute_free else _line(needle))
    return {"item_id": f"V11b-{cond}-s{seed}", "cond": cond, "seed": seed, "compute_free": compute_free,
            "truth": needle["final"],
...
def _oracle(seed):
    ...
    dsing = [c["final"] for c in st["chains"] if _is_wurf(c["final"])]
...
def selftest(seeds):
    ...
    for s in range(seeds):
        ...
        dw = [len(DEF_PRIME.format(W='PRIME').split()), len(DEF_PRIME.format(W='FLONK').split()),
              len(DEF_NOVEL.format(W='ZILP').split())]
        if max(dw) - min(dw) > 1: bad.append((s, f"def word drift {dw} (must be matched)"))
...
def write_lock(seeds):
    ...
    (HERE / "v11b_labels.jsonl").write_text("\n".join(json.dumps(l, ensure_ascii=False) for l in labels) + "\n", encoding="utf-8")
    (HERE / "v11b_labels.LOCK").write_text(json.dumps(
        {"sha256": digest, "n_items": len(labels), "seeds": seeds, "conds": CONDS, "tier": TIER,
         "note": "V11b matched novel predicate de-amortization; headline e(NOVEL_RULE)-e(NAMED_DEF)"},
        indent=2) + "\n", encoding="utf-8")
    print(f"LOCKED {len(labels)} items sha256={digest[:16]}... -> v11b_labels.jsonl + v11b_labels.LOCK")


def run(seeds, repeats, workers):
    import numpy as np
    lk = json.loads((HERE / "v11b_labels.LOCK").read_text(encoding="utf-8"))
    ...
    print(f"=== V11b de-amortize (matched): {len(use)} items x {repeats} reps = {len(jobs)} calls on '{MODEL}' ({workers} workers) ===")
...
def analyze(stream, seeds, repeats):
    ...
    delta("NOVEL_RULE", "NAMED_DEF", "(1) HEADLINE -- un-amortized selection concept (compute matched)")
    print("  THE FALSIFIER (compute removed both sides, length-matched):")
    delta("F0_NOVEL_RULE", "F0_NAMED_DEF", "vanishes (CI~0) => headline was concept-APPLICATION")
    print(f"  wrote v11b_run.{MODEL}.jsonl ({len(stream)} records)")
...
if __name__ == "__main__":
    ...
    if a.selftest: selftest(a.seeds)
        ...
        stream = [json.loads(l) for l in (HERE / f"v11b_run.{MODEL}.jsonl").read_text(encoding="utf-8").splitlines() if l.strip()]
